	...
	WITH DScreen.Obj[i] DO
		...
		if (SpriteFill & DScreen.Obj[i].Flags) {
			...
			if (Centered & DScreen.Obj[i].Flags)
				PrintAt(ox + DScreen.Obj[i].w / 2 - Length(Text) / 2, oy, DScreen.Obj[i].Text);
			else
				PrintAt(ox, oy , DScreen.Obj[i].Text);
			TextMode = 0;
		} else {
			if (Centered & Flags)
				PrintAt(2 * ox + DScreen.Obj[i]..w - Length(Text) / 2, oy, DScreen.Obj[i].Text);
			else
...
unsigned  EditObject(unsigned e, char ch)
{
	int ok;
	if (ch >= ' ') {
